## UN Handbook - Chapter 8: Quality control of training sets for agricultural statistics
...
```shell
git clone https://github.com/FAO-EOSTAT/un-handbook-th-quality
```
...
```shell
docker pull faoeostat/handbook:chp8
```
...
```shell
docker build -t "faoeostat/handbook:chp8" --file docker/Dockerfile .
```
...
docker run \
    ...
    --volume ${PWD}:/home/rstudio/handbook faoeostat/handbook:chp8
```
...
Once you have accessed RStudio in your browser, you need to open the project. The project files are available in the `handbook` directory, which is located in the RStudio user home directory. Navigate to this directory using the Files pane in RStudio. Then double-click on the `th_quality_control.Rproj` file to open the project. This will configure RStudio with the correct working directory and project settings.
